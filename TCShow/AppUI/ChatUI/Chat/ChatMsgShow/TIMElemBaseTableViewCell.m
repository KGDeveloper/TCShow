...
- (void)configWith:(IMAMsg *)msg
{
    ...
    if ([_msg isMineMsg])
    {
        user = (IMAUser *)[IMAPlatform sharedInstance].host;
        iconPath = [SARUserInfo headUrl];
    }
    else
    {
        user = [_msg getSender];
        NSUserDefaults * chatUser = [NSUserDefaults standardUserDefaults];
        if ([chatUser objectForKey:@"chatList"]) {
            NSDictionary * chatListDic = [chatUser objectForKey:@"chatList"];
            if ([chatListDic objectForKey:user.userId]) {
                NSDictionary * userDic = [chatListDic objectForKey:user.userId];
                iconPath = IMG_APPEND_PREFIX(userDic[@"userIcon"]);
            }
        }
    }
    [_icon sd_setImageWithURL:[NSURL URLWithString:iconPath] forState:UIControlStateNormal placeholderImage:kDefaultUserIcon];
