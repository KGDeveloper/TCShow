...
@interface SDContactsViewController ()<UITableViewDelegate,UITableViewDataSource>{
...
@implementation SDContactsViewController
...
- (void)viewDidLoad {
    ...
    self.tableView = [[UITableView alloc]initWithFrame:CGRectMake(0, 0,kSCREEN_WIDTH, kSCREEN_HEIGHT - 64 - 49) style:UITableViewStyleGrouped];
...
- (void)genDataWithCount{
    [[Business sharedInstance] friendList:[SARUserInfo userId] succ:^(NSString *msg, id data) {
        if ([data[@"code"] integerValue] == 0) {
            _dataArray = [TCLiveUserList mj_objectArrayWithKeyValuesArray:data[@"data"] context:nil];
            [self setUpTableSection];
            ...
            if ([data[@"code"] integerValue] == -2) {
                ...
                [SARUserInfo removeContactsList];
                [self setUpTableSection];
                [_tableView reloadData];
            }
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    TCLiveUserList *model = self.sectionArray[section][row];
    cell.model = model;
    return cell;
}
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    TCLiveUserList * model= self.sectionArray[indexPath.section][indexPath.row];
    model.is_live = @"0";
    model.province = @"";
