...
+ (void)jumpToWxPayWithMoney:(NSString *)money Order:(NSString *)order type:(NSInteger)type
{
    //============================================================
    // 支付流程实现
    // 客户端操作     (实际操作应由服务端操作)
    ...
    NSString *notifyUrl;// 交易结果通知网站此处用于测试，随意填写，正式使用时填写正确网站
    if (type == 1) {
        notifyUrl = @"http://47.93.32.34:8081/index.php/appapi/pay/wxnotify";
    }else {
        notifyUrl = @"http://47.93.32.34:8081/index.php/appapi/pay/wxnotify2";
    }
    ...
    [session POST:kUrlWechatPay parameters:string success:^(NSURLSessionDataTask *task, id responseObject) {
        //  输出XML数据
        ...
        if ([[dic objectForKey:@"result_code"] isEqualToString:@"SUCCESS"]
            ...
            [[HUDHelper sharedInstance] tipMessage:dic[@"err_code_des"]];
        }
