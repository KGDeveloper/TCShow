...
//用户礼物缓存池
@property (nonatomic, strong) NSCache *userGiftCache;
@end
...
//    if ([self.userGiftCache objectForKey:[NSString stringWithFormat:@"%ld", model.type]]) {
//        
//    }
    // 在有用户礼物信息时
    if ([self.userGigtInfos objectForKey:userID]) {
            ...
            if ([self.operationCache objectForKey:userID]!=nil) {
                if ([[self.userGiftCache objectForKey:[NSString stringWithFormat:@"%ld", (long)model.type]] integerValue] == model.type) {
                    ...
                    AnimOperation *op = [self.operationCache objectForKey:userID];
                    op.presentView.giftCount = model.giftCount;
                    [op.presentView changePresentViewUIWithModel:model];
                    [op.presentView shakeNumberLabel];
                    [self.userGiftCache setObject:[NSString stringWithFormat:@"%ld", (long)model.type] forKey:[NSString stringWithFormat:@"%ld", (long)model.type]];
                }
//                AnimOperation *op = [self.operationCache objectForKey:userID];
//                op.presentView.giftCount = model.giftCount;
//                [op.presentView shakeNumberLabel];
        ...
        AnimOperation *op = [AnimOperation animOperationWithUserID:userID model:model finishedBlock:^(BOOL result,NSInteger finishCount) {
            ...
            [self.userGiftCache removeObjectForKey:[NSString stringWithFormat:@"%ld", (long)model.type]];
            // 延时删除用户礼物信息
...
- (NSCache *)userGiftCache {
    if (_userGiftCache == nil) {
        _userGiftCache = [[NSCache alloc] init];
    }
    return _userGiftCache;
}
