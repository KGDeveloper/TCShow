...
{
//        IMAConversationShowAble
    NSArray  * userList = [[SARUserInfo gainContacts] objectForKey:@"data"];
    _conversationName.text = [_showItem showTitle];
    for (NSDictionary * dic in userList) {
        if ([dic[@"phone"]isEqualToString:[_showItem showTitle]]) {
            _conversationName.text = [dic objectForKey:@"nickname"];
            [_conversationIcon sd_setBackgroundImageWithURL:[NSURL URLWithString:IMG_APPEND_PREFIX([dic objectForKey:@"headsmall"])] forState:UIControlStateNormal placeholderImage:[_showItem defaultShowImage]];
        }
    }
    NSDictionary * userDic = [[NSUserDefaults standardUserDefaults] objectForKey:@"chatList"];
    if ([userDic objectForKey:[_showItem showTitle]]) {
        NSDictionary * userNameDic = [userDic objectForKey:[_showItem showTitle]];
        _conversationName.text = [userNameDic objectForKey:@"userName"];
        [_conversationIcon sd_setBackgroundImageWithURL:[NSURL URLWithString:IMG_APPEND_PREFIX([userNameDic objectForKey:@"userIcon"])] forState:UIControlStateNormal placeholderImage:[_showItem defaultShowImage]];
    }
